...
I used 80% of the avaiable data for train the model. The reamining 20% will be used as test dataset for evaluating the developed model.

For the developed model is very important to detect poisonus mushrooms to avoid possible health damages or death. Therefore the model performance will be evaluated with the F1 score instead of the overal accuracy. Because the models use the edible prediction as true value, I will try to get a high specificity with the developed models..
...
For testing the preformance the predition model we have about 1,600 mushrooms data (20% of all data)
...
We can not classify only the mushrooms with the odor value 'n' (none). There are 2,809 mushrooms remaining in the dataset without classification. 
...
```{r classification_per_feature_without_odor, warning=FALSE, error=FALSE, message=FALSE, fig.width=10, fig.height=15}
...
We can not make a classification only it the spore-print-color value is 'w' (white). 
...
```{r classification_per_feature_without_spore_print_color, warning=FALSE, error=FALSE, message=FALSE, fig.width=10, fig.height=15}
...
We can repeate this analysis and look for features that we can use for classifying the remaining data set.
As next step we can get the gill-color feature and examine only the mushrooms with the grill color 'w' (white).

```{r classification_per_feature_gill_color_w, warning=FALSE, error=FALSE, message=FALSE, fig.width=10, fig.height=15}
...
```{r classification_per_feature_gill_size_n, warning=FALSE, error=FALSE, message=FALSE, fig.width=10, fig.height=15}
...
```{r classification_per_feature_stalk_surface_below, warning=FALSE, error=FALSE, message=FALSE, fig.width=10, fig.height=15}
...
The remaining dataset has only 25 mushrooms. As we can see on the plot, now we can use some feature for classifying this remaining mushrooms.
...
The analyse above give us an other important information: some fatures are more relevant for the classification some features gives no additional information for the method.
That means, the different features have different information weight for the classification.

We have to try to calculate a correlation between the features and the edibility.
For the unordered categorical features we can use the uncertinity coefficient as correlation score. (See: <https://en.wikipedia.org/wiki/Uncertainty_coefficient>)
...
```{r uncertinty_coefficient_functions, warning=FALSE, error=FALSE, message=FALSE, fig.width=10, fig.height=15}
...
We can plot the uncertinity coefficient between all the features as a matrix. The following R code creates the plot:
...
```
This plot shows the uncertinity coefficient between all feature pairs. The light blue values show a high uncertinity coefficient (high correlation) the dark blue values shows a low uncertinity coefficient (low correlation)
...
The $F_1$ score is the harmonic average of the precision and reacall :
...
As we saw before, we can develop a prediction model based on the feature distribution between the edible and poisonus classes.
...
For this method we can use the same logik as we did:

1. create the edibility distribution for all features for the dataset
2. search for features, that have values with only either 'e' or 'p' edibility. If we don't find a feature, than we cann't improve our decision tree anymore
...
For the prediction, we can go throug the features in the prediction tree and check if the feature in the actual record matches either the feature values with dictinct 'e' or 'p' edibility. If we find a match, we use the corresponding edibility value as prediction.
...
The uncertinity score matrix give us an overview about the correlation between the edibility classification and other features.
...
For example, we can use the featuers 'odor' and 'spore-print-color' and generate a summary for all existing feature combination to this two featue und the edibility classification.
...
There are some odor/spore-print-color value combination that have both 'p' and 'e' edibitlity classification.
for this combination we don't have a definitly prediction. If we want to have a high specificity (we want to predict the poisonous mushrooms as good as possible), we have to predict 'p' in this case.

There are some odor/spore-print-color value combination with exclusive 'p' edibitlity classification. In this case we can predict 'p'.

There are some odor/spore-print-color value combination with exclusive 'e' edibitlity classification. In this case (and only in this case) we can predict, that the mushroom is edible.

Our prediciton model will search all feature value combination with exlusive 'e' edibility form the train set and predict for any record from the test set that mach to one of this value combination 'e' edibility. For all other records in the test set we predict 'p' edibility.
...
After a specific number of used feature we will get worse $F_1$ score again. That is the effect of the overtraining. Therefore we have to find out the nuber of the features to use for the best prediction performance. 


\newpage
# Implementation & result

In this chapter i will implement the definied two prediction model and analyse the result.
...
As first iteration I implemented a naive decision tree model based on the analysis in the previos chapters.
This implementation is not trainable, it depends on the obervation about the train dataset.
The R implementation this naive decision tree is wie follow:
...
The funcion has to chekc the first decision rules existing in the decision tree and check if the actual record match either the values for the edible or the poisonous classification. If there is no match, the function have to go through the following rules until a match is found or the decision tree has no more rules.
